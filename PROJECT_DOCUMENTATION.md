...
- Не может бронировать слот, потому что POST-бронирование разрешено только роли `patient`.

### Admin

- Может войти в систему и открыть кабинет.
- Специальных маршрутов управления пользователями пока нет.
...
| POST | `/slots/{slot_id}/book` | Бронирование слота | Patient |
...
1. Проверяется сессия: бронировать может только пациент.
2. Находится `patient_id` по email.
...
10. Войти вторым пациентом, если он добавлен в seed или вручную, и попробовать занять уже забронированный слот. Должно появиться сообщение `unavailable`.
11. Войти как `doc@clinic.local` и открыть `My appointments`. Врач должен видеть только записи к себе.
...
## 14. Полная очистка Docker

Полная очистка неиспользуемых Docker-объектов:

```bash
docker system prune -a --volumes --force
```

Если нужно удалить даже работающие контейнеры перед очисткой:
...
После этого Docker удалит контейнеры, images, named volumes, неиспользуемые networks и build cache. Системные сети `bridge`, `host` и `none` обычно останутся.

## 15. Git и данные базы
...
- В login-шаблоне перечислены основные тестовые аккаунты, но `doc2@clinic.local` добавлен в seed и может быть введен вручную.
- Внешний вид остается минимальным учебным HTML без отдельной дизайн-системы.
- В приложении нет CSRF-защиты для POST-бронирования.
...
## 17. Быстрый reset и запуск с нуля
