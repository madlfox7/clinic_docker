...
14. Для 3 октября проверить overlap в обоих направлениях: Anna `10:00-11:00` и Levon `10:30-11:30`.
15. Войти как registrar и забронировать за `pat2@clinic.local`; проверить строку в `/my` пациента и расписании врача. Проверить overlap, пустой email, email врача и несуществующий email.
16. Войти как admin, заблокировать `pat2`, проверить запрет входа и registrar-booking, затем разблокировать. Блокировка не должна удалять существующие записи.
17. Выполнить `docker compose down` и `docker compose up -d` без `--volumes`; убедиться, что appointments сохранились.
