...
10. Нажать `Cancel appointment` в `/my`. Запись должна перейти в `cancelled` и перестать блокировать время. После отмены слот должен сразу показываться как свободный в списке слотов.
